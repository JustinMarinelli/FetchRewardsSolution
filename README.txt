...
The data is stored at the given URL with 3 fields per row - id, listId, and name. I am aware that for rows that have non-null and non-empty names, the id seems a bit redundant as it is already included in the name, for example: 
{"id":0, "listId": 1, "name": "Item 0"} 
However, I did not want to omit the ID field for a specific reason. If this software were to be used in the future for another data set, perhaps there could be a row that has another form, such as: 
{"id":1, "listId":1, "name": "DifferentItem 0"}. 
Notice how the id and the number in the name are not equal. Because of this conern, I decided to include all fields in the View.
